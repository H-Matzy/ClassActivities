...
let cities = [];
data.map(city => {
  cities.push(city.address.city);
});
console.log(cities);
// create an array of latitude and longitude coordinate pairs
let cords = [];
data.map(ele => {
  cords.push(`Lat: ${ele.address.geo.lat} Lng: ${ele.address.geo.lat}`);
});
console.log(cords);
// create an array of strings that combine the company's catchPhrase and bs
let companyBS = [];
data.map(ele => {
  companyBS.push(ele.company.catchPhrase + ": " + ele.company.bs);
});
console.log(companyBS);
