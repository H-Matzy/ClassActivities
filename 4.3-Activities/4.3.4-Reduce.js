...
let completedTallyObject = tasks.reduce(
  (acc, task) => {
    if (task.completed === true) {
      acc.completedTasks += 1;
    } else {
      acc.incompleteTasks += 1;
    }
    return acc;
  },
  {
...
// let status = tasks.reduce((acc, curr) => {
...
let numsToSum = arrayOfNumbers(200);
// using reduce, sum the numbers from 1 to 200
let theSum = numsToSum.reduce((acc, num) => (acc += num), 0);
console.log(theSum);
